...
def clean_shopify_data(df):
    ...
    for order_num in unique_order_numbers:
        ...
        if len(order_rows) >= 2:
            ...
            print(
                f"Order #{order_num}: Merged {len(order_rows)} rows, kept amended Lineitem data"
            )
...
def filter_international_orders(df):
    ...
    intl_orders = df[
        (df["Shipping Country"] != "SG")
        & (df["Shipping Country"] != "US")
        & (df["Shipping Country"] != "CA")
    ].copy()
...
def parse_product_details(lineitem_name):
    ...
    is_bundle = (
        "BUNDLE" in lineitem_name
        or "2 PAIRS" in lineitem_name
        or "BUNDLE OF 2" in lineitem_name
    )
...
def fetch_usd_prices_from_shopify(us_orders):
    ...
    if not access_token or not store_url:
        raise Exception(
            "Shopify API credentials not configured. Please add Shopify credentials (access_token and store_url) to .streamlit/secrets.toml"
        )
    ...
    for _, row in us_orders.iterrows():
        ...
        try:
            ...
            if data.get("orders") and len(data["orders"]) > 0:
                order = data["orders"][0]
                usd_amount = order["current_subtotal_price_set"]["presentment_money"][
                    "amount"
                ]
            ...
            error_msg = (
                f"Error connecting to Shopify API for order #{row['Name']}: {str(e)}"
            )
            raise Exception(error_msg)
        except KeyError as e:
            error_msg = (
                f"Unexpected API response format for order #{row['Name']}: {str(e)}"
            )
            raise Exception(error_msg)
...
def create_us_singpost_row(row, is_bundle, material, size, hs_code, usd_price):
    ...
    address_line_2 = safe_str_slice(row["Shipping Address2"], 35).strip() or "NA"
    ...
    us_row = {
        ...
        "Send to business name (Max 35 characters) - *": safe_str_slice(
            row["Shipping Name"], 35
        ),
        "Send to contact person (Max 35 characters)": safe_str_slice(
            row["Shipping Name"], 35
        ),
        "Send to address line 1 eg. Block no. (Max 35 characters) - *": safe_str_slice(
            row["Shipping Address1"], 35
        ),
        ...
        "Send to town (Max 30 characters) (Please spell in full)": safe_str_slice(
            row["Shipping City"], 30
        ),
        "Send to state (Max 30 characters) (Please spell in full)": state,
        "Send to country (Max 2 characters) - * (Refer to Country List sheet)": safe_str_slice(
            row["Shipping Country"], 2
        ),
        "Send to postcode (Max 10 characters)": safe_str_slice(
            str(row["Shipping Zip"]), 10
        ).replace("'", ""),
        "Send to phone no. (Max 20 characters)": safe_str_slice(
            row["Shipping Phone"], 20
        )
        if pd.notna(row["Shipping Phone"])
        else "",
...
def convert_shopify_to_singpost(shopify_file, output_file):
    ...
    if not os.path.exists(shopify_file):
        return (
            f"Error: Input file '{shopify_file}' not found. Please check the file path.",
            None,
            None,
        )
    ...
    for region_name, region_df, counter, details in [
        ...
        for _, row in region_df.iterrows():
            ...
            if region_name == "Singapore":
                detail.update(
                    {
                        "address1": safe_str_slice(row["Shipping Address1"], 50)
                        if pd.notna(row["Shipping Address1"])
                        else "",
                        "address2": safe_str_slice(row["Shipping Address2"], 50)
                        if pd.notna(row["Shipping Address2"])
                        else "",
                        "city": safe_str_slice(row["Shipping City"], 30)
                        if pd.notna(row["Shipping City"])
                        else "",
                        "postal": safe_str_slice(str(row["Shipping Zip"]), 10)
                        if pd.notna(row["Shipping Zip"])
                        else "",
                        "phone": safe_str_slice(row["Shipping Phone"], 20)
                        if pd.notna(row["Shipping Phone"])
                        else "",
                    }
                )
            ...
            if region_name == "US":
                ...
                us_row = create_us_singpost_row(
                    row, is_bundle, material, size, hs_code, usd_price
                )
            ...
            # Create SingPost entries for International orders (EXISTING format - UNCHANGED)
                ...
                # Handle Address Line 2
                address_line_2 = (
                    safe_str_slice(row["Shipping Address2"], 35).strip() or "NA"
                )

                # Check for address truncation
                if (
                    pd.notna(row["Shipping Address1"])
                    and len(str(row["Shipping Address1"])) > 35
                    or pd.notna(row["Shipping Address2"])
                    and len(str(row["Shipping Address2"])) > 35
                ):
                ...
                simplified_description = (
                    f"Eczema Bolero Shrug {quantity_str}{size_str} {material}"
                )

                singpost_row = {
                    "Send to business name line 1 (Max 35 characters) - *": safe_str_slice(
                        row["Shipping Name"], 35
                    ),
                    "Send to business name line 2 (Max 35 characters)": safe_str_slice(
                        row["Shipping Phone"], 35
                    )
                    if pd.notna(row["Shipping Phone"])
                    else "",
                    "Send to address line 1 (Max 35 characters) - *": safe_str_slice(
                        row["Shipping Address1"], 35
                    ),
                    "Send to address line 2  (Max 35 characters) - *": address_line_2,
                    "Send to address line 3 (Max 35 characters)": safe_str_slice(
                        row["Email"], 35
                    )
                    ...
                    "Send to state (Max 30 characters) (Please spell in full)": state,
                    "Send to country (Max 2 characters) - *": safe_str_slice(
                        row["Shipping Country"], 2
                    ),
                    "Send to postcode (Max 10 characters)": safe_str_slice(
                        str(row["Shipping Zip"]), 10
                    ...
                    "Sender VAT/GST number (Max 50 characters)": "",
                    "Sender Reference (Max 20 characters)": safe_str_slice(
                        row["Name"], 20
                    ),
    ...
    def print_region_breakdown(region_name, product_counter, order_details):
        ...
        cotton_products = {
            k: v for k, v in product_counter.items() if k.startswith("Cotton")
        }
        tencel_products = {
            k: v for k, v in product_counter.items() if k.startswith("Tencel")
        }
        unknown_products = {
            k: v
            for k, v in product_counter.items()
            if not (k.startswith("Cotton") or k.startswith("Tencel"))
        }
    ...
    summary += print_region_breakdown(
        "INTERNATIONAL", intl_product_counter, intl_order_details
    )

    # Calculate grand totals
    total_orders = (
        len(sg_order_details)
        + len(us_order_details)
        + len(ca_order_details)
        + len(intl_order_details)
    )
    total_pieces = (
    ...
    if intl_singpost_data:
        ...
        for col in required_columns:
            ...
            if missing > 0:
                print(
                    f"WARNING: {missing} rows have missing values in required field: {col}"
                )
    ...
    if sg_order_details:
        ...
        if credentials_path and os.path.exists(credentials_path):
            try:
                template_id = (
                    get_template_id_from_url(template_url) if template_url else None
                )
                if template_id:
                    slides_url = create_shipping_slides(
                        sg_order_details, credentials_path, template_id
                    )
                    if slides_url:
                        summary += (
    ...
    return (
        ...
        us_df,
        slides_url,
    )  # Return summary, international DF, US DF, and Slides URL


# Example usage
if __name__ == "__main__":
    result, pdf_path, slides_url = convert_shopify_to_singpost(
        "orders_export.csv", "singpost_orders.csv"
    )
    print(result)
    if slides_url:
